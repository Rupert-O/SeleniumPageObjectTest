...
//        }//TEMPORARY !!
        ...
        for(WebElement i: results.getProductsTitles()){
            if(pattern.matcher(i.getText()).find()){//if string match to REGEX
        ...
        Assert.assertEquals(productTitlesWithKeyWord, results.getProductsTitles().size(), "Not every product contains key word");
    }
}